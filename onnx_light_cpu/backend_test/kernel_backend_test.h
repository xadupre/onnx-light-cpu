...
// onnx-light registry. It does **not** run or check anything itself: the cases
// are executed and validated by the unit tests, which register them, register
// the kernels and then drive them through onnx-light's regular API
// (``CollectTestCases`` + the runtime in C++, ``collect_test_cases`` +
// ``ReferenceEvaluator`` in Python).
