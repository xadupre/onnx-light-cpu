...
ONNX_LIGHT_CPU_NOINLINE double ComputeArithmeticAvxFloat32Round(std::size_t passes, double seed) {
  __m256 acc[kComputeAvxRegisters];
  __m256 mul[kComputeAvxRegisters];
  __m256 add[kComputeAvxRegisters];
  for (int reg = 0; reg < kComputeAvxRegisters; ++reg) {
  ...
  for (std::size_t pass = 0; pass < passes; ++pass) {
    for (std::size_t chain = 0; chain < kComputeChainLength; ++chain) {
      for (int reg = 0; reg < kComputeAvxRegisters; ++reg) {
        acc[reg] = _mm256_add_ps(add[reg], _mm256_mul_ps(acc[reg], mul[reg]));
      }
    }
  }
...
ONNX_LIGHT_CPU_NOINLINE double ComputeArithmeticAvxFloat64Round(std::size_t passes, double seed) {
  __m256d acc[kComputeAvxRegisters];
  __m256d mul[kComputeAvxRegisters];
  __m256d add[kComputeAvxRegisters];
  for (int reg = 0; reg < kComputeAvxRegisters; ++reg) {
  ...
  for (std::size_t pass = 0; pass < passes; ++pass) {
    for (std::size_t chain = 0; chain < kComputeChainLength; ++chain) {
      for (int reg = 0; reg < kComputeAvxRegisters; ++reg) {
        acc[reg] = _mm256_add_pd(add[reg], _mm256_mul_pd(acc[reg], mul[reg]));
      }
    }
  }
