...
public:
  ...
  std::int32_t data_type() const noexcept { return data_type_; }
...
private:
  ElementwiseFusionPlan(ElementwiseFusionTemplate fusion_template, std::int32_t data_type,
                        std::span<const std::int64_t> output_shape);

  ElementwiseFusionTemplate fusion_template_;
  std::int32_t data_type_;
