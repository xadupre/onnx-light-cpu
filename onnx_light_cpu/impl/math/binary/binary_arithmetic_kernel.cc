...
using detail::BinaryBitShift_Scalar;
using detail::ShiftInputMode;
#ifdef ONNX_LIGHT_CPU_HAVE_AVX2
using detail::BinaryBitShift_AVX2;
using detail::BinaryBitShiftHasInvalidAmount_AVX2;
#endif
...
// ---------------------------------------------------------------------------
// SSE2 (128-bit) implementations. The intrinsic names
// (``_mm_<op>_p[sd]``) are regular across Add/Sub/Mul/Div, so a single
...
#define ONNX_LIGHT_CPU_BIN_AVX_PTR(STEM) (&STEM##_AVX)
#define ONNX_LIGHT_CPU_BIN_AVX_LEFT_PTR(STEM) (&STEM##Left_AVX)
#define ONNX_LIGHT_CPU_BIN_AVX_RIGHT_PTR(STEM) (&STEM##Right_AVX)
#else
#define ONNX_LIGHT_CPU_BIN_AVX_PTR(STEM) (nullptr)
#define ONNX_LIGHT_CPU_BIN_AVX_LEFT_PTR(STEM) (nullptr)
#define ONNX_LIGHT_CPU_BIN_AVX_RIGHT_PTR(STEM) (nullptr)
#endif
...
inline Fn PickImpl(Fn scalar_fn, Fn sse2_fn, Fn avx_fn, Fn avx512_fn, Fn neon_fn, Fn sve_fn) {
#if ONNX_LIGHT_CPU_BINARY_X86
  (void)neon_fn;
  (void)sve_fn;
  ...
  if (avx_fn != nullptr && level >= SimdLevel::kAVX) {
    return avx_fn;
  }
  ...
  void PUBLIC##Contiguous(const T *left, const T *right, T *out, std::size_t count) {              \
    ...
    static const Fn fn =                                                                           \
        PickImpl<Fn>(&STEM##_Scalar, ONNX_LIGHT_CPU_BIN_SSE2_PTR(STEM),                            \
                     ONNX_LIGHT_CPU_BIN_AVX_PTR(STEM), ONNX_LIGHT_CPU_BIN_AVX512_PTR(STEM),        \
...
#define ONNX_LIGHT_CPU_BIN_DISPATCH_LEFT(PUBLIC, STEM, T)                                          \
  void PUBLIC##LeftScalar(T left, const T *right, T *out, std::size_t count) {                     \
    ...
    static const Fn fn = PickImpl<Fn>(                                                             \
        ...
        ONNX_LIGHT_CPU_BIN_AVX_LEFT_PTR(STEM), ONNX_LIGHT_CPU_BIN_AVX512_LEFT_PTR(STEM),           \
...
#define ONNX_LIGHT_CPU_BIN_DISPATCH_RIGHT(PUBLIC, STEM, T)                                         \
  void PUBLIC##RightScalar(const T *left, T right, T *out, std::size_t count) {                    \
    ...
    static const Fn fn = PickImpl<Fn>(                                                             \
        ...
        ONNX_LIGHT_CPU_BIN_AVX_RIGHT_PTR(STEM), ONNX_LIGHT_CPU_BIN_AVX512_RIGHT_PTR(STEM),         \
...
void BinaryPReluFloat32Contiguous(const float *left, const float *right, float *out,
  ...
  static const Fn fn =
      PickImpl<Fn>(&BinaryPReluFloat32_Scalar, ONNX_LIGHT_CPU_BIN_SSE2_PTR(BinaryPReluFloat32),
                   ONNX_LIGHT_CPU_BIN_AVX_PTR(BinaryPReluFloat32),
...
void BinaryPReluFloat32LeftScalar(float left, const float *right, float *out, std::size_t count) {
  ...
  static const Fn fn = PickImpl<Fn>(&BinaryPReluFloat32Left_Scalar,
                                    ...
                                    ONNX_LIGHT_CPU_BIN_AVX_LEFT_PTR(BinaryPReluFloat32),
...
void BinaryPReluFloat32RightScalar(const float *left, float right, float *out, std::size_t count) {
  ...
  static const Fn fn = PickImpl<Fn>(
      ...
      ONNX_LIGHT_CPU_BIN_AVX_RIGHT_PTR(BinaryPReluFloat32),
