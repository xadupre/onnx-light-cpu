...
_UNARY_ORT_TYPES = {
    "Abs": (
        (TensorProto.FLOAT, np.float32, range(1, 27)),
        (TensorProto.DOUBLE, np.float64, range(1, 27)),
        (TensorProto.FLOAT16, np.float16, range(1, 27)),
        (TensorProto.BFLOAT16, ml_dtypes.bfloat16, range(13, 27)),
        (TensorProto.INT8, np.int8, range(6, 27)),
        (TensorProto.INT16, np.int16, range(6, 27)),
        (TensorProto.INT32, np.int32, range(6, 27)),
        (TensorProto.INT64, np.int64, range(6, 27)),
    ),
    "Exp": (
        (TensorProto.FLOAT, np.float32, range(1, 27)),
        (TensorProto.DOUBLE, np.float64, range(1, 27)),
    ...
    "Not": ((TensorProto.BOOL, np.bool_, range(1, 27)),),
}


def _unary_ort_values(op_type, dtype, shape):
    ...
    if dtype == np.bool_:
        ...
        info = np.iinfo(dtype)
        values = [info.min, -3, -1, 0, 1, 3, info.max]
    elif op_type == "Log":
        values = [
            -np.inf,
            -1.0,
        ...
        values = [-np.inf, -100.0, -1.0, -0.0, 0.0, 1.0, 80.0, np.inf, np.nan]
    return np.resize(np.asarray(values, dtype=dtype), count).reshape(shape)


def _make_unary_ort_model(op_type, tensor_type, shape, opset):
    graph = helper.make_graph(
        [helper.make_node(op_type, ["X"], ["Y"])],
        f"{op_type.lower()}_onnxruntime_parity",
        [helper.make_tensor_value_info("X", tensor_type, shape)],
        [helper.make_tensor_value_info("Y", tensor_type, shape)],
    )
    return helper.make_model(
        graph, opset_imports=[helper.make_opsetid("", opset)], ir_version=_ORT_MODEL_IR_VERSION
    )


def _unary_ort_tolerance(op_type, dtype):
    if op_type == "Abs":
        return 0.0
...
class TestBackendCases(ExtTestCase):
    ...
    def test_optimized_unary_kernels_match_onnx_runtime(self):
        import onnxruntime as ort

        options = ort.SessionOptions()
        options.intra_op_num_threads = 1
        options.inter_op_num_threads = 1
        for op_type, type_cases in _UNARY_ORT_TYPES.items():
            for tensor_type, dtype, opsets in type_cases:
                for opset in opsets:
                    for shape in ((), (0,), (1,), (3, 3), (17,)):
                        with self.subTest(op_type=op_type, dtype=dtype, opset=opset, shape=shape):
                            model = _make_unary_ort_model(op_type, tensor_type, shape, opset)
                            feeds = {"X": _unary_ort_values(op_type, dtype, shape)}
                            clear_used_kernel_names()
                            ...
                            if op_type == "Not" or (
                                op_type == "Abs" and np.issubdtype(dtype, np.integer)
                            ):
                                np.testing.assert_array_equal(actual, expected)
                            else:
        ...
        for op_type in ("Abs", "Exp", "Log"):
            with self.subTest(op_type=op_type, execution="parallel"):
                shape = (524288,)
                model = _make_unary_ort_model(op_type, TensorProto.FLOAT, shape, 26)
                feeds = {"X": _unary_ort_values(op_type, np.float32, shape)}
                clear_used_kernel_names()
                actual = ReferenceEvaluator(model).run(None, feeds)[0]
                expected = ort.InferenceSession(
                    model.SerializeToString(),
