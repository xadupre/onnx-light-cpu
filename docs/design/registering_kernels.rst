...
The shipped ``Abs``, ``Exp``, ``Log``, ``Gemm``, ``MatMulInteger``,
``QLinearMatMul`` and ``Not`` kernels are installed with a single call.
...
.. code-block:: cpp
    ...
    onnx_light_cpu::RegisterAllKernels();  // Abs/Exp/Log/Gemm/MatMulInteger/QLinearMatMul/Not now use the SIMD kernels

Or register a single operator with the per-operator functions
(``RegisterAbsKernel()``, ``RegisterExpKernel()``, ``RegisterLogKernel()``,
``RegisterGemmKernel()``, ``RegisterIntegerMatMulKernels()`` — which installs
both the ``MatMulInteger`` and ``QLinearMatMul`` kernels — and
``RegisterNotKernel()``).
...
``"onnx_light_cpu::Log"``, ``"onnx_light_cpu::Gemm"``,
``"onnx_light_cpu::MatMulInteger"``, ``"onnx_light_cpu::QLinearMatMul"``,
``"onnx_light_cpu::Not"``) that it records every time it runs. The names can be
inspected from Python:
